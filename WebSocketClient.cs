...
public class WebSocketClient
{
    ...
    private async void RunSend()
    {
        ...
        while (true)
        {
            while (!sendQueue.IsCompleted)
            {
                ...
                await ws.SendAsync(msg, WebSocketMessageType.Text, false, CancellationToken.None);
            }
        }
    }
    #endregion
    ...
    private async Task<string> Receive(UInt64 maxSize = MAXREADSIZE)
    {
        ...
        if (IsConnectionOpen())
        {
            Debug.Log("IsConnectionOpen not");
            do
            {
